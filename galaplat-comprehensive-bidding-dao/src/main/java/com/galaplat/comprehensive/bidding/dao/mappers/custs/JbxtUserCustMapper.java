...
public interface JbxtUserCustMapper extends JbxtUserMapper {
    ...
    @Insert({"<script>",
            ...
            "updator, creator, ",
            "company_code, sys_code, activity_code, supplier_name, code_name,contact_person,phone," ,
            "email_address,login_status, send_sms,send_mail) ",
            "values ",
            ...
            "#{item.activityCode,jdbcType=VARCHAR}, #{item.supplierName,jdbcType=VARCHAR}, ",
            "#{item.codeName,jdbcType=VARCHAR},#{item.contactPerson,jdbcType=VARCHAR},#{item.phone,jdbcType=VARCHAR}," ,
            "#{item.emailAddress,jdbcType=VARCHAR},#{item.loginStatus,jdbcType=INTEGER},#{item.sendSms,jdbcType=INTEGER}," ,
            "#{item.sendMail,jdbcType=INTEGER})",
    ...
    @Select({"<script>",
            ...
            " from t_jbxt_user ",
            " where 1=1 ",
            "<if test='param.code != null and param.code != \"\"' > and code = #{param.code,jdbcType=VARCHAR}</if>",
            "<if test='param.supplierNames != null and param.supplierNames.size() > 0' > " ,
            "  and supplier_name in " ,
            " <foreach item='item' index='index' collection='param.supplierNames' open='(' separator=',' close=')'> ",
            "  #{item} ",
            " </foreach>",
            "</if>",
    ...
    @Select({"<script>",
            ...
            "<if test='param.activityCode != null' > and activity_code = #{param.activityCode,jdbcType=VARCHAR}</if>",
            "<if test='param.supplierName != null' > and supplier_name = #{param.supplierName,jdbcType=VARCHAR}</if>",
            "<if test='param.codeName != null' > and code_name = #{param.codeName,jdbcType=VARCHAR}</if>",
            "<if test='param.contactPerson != null' > and contact_person = #{param.contactPerson,jdbcType=VARCHAR}</if>",
            "<if test='param.phone != null' > and phone = #{param.phone,jdbcType=VARCHAR}</if>",
            "<if test='param.emailAddress != null' > and email_address = #{param.emailAddress,jdbcType=VARCHAR}</if>",
            "</script>",
    })
    @Results({
            @Result(column = "code", property = "code", jdbcType = JdbcType.VARCHAR, id = true),
